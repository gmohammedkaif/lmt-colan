...
// mobileMode: rendered inside the mobile drawer (no hidden lg:flex wrapper needed)
// onLinkClick: called after a nav link is clicked (to close the drawer)
function Sidebar({ mobileMode = false, onLinkClick }) {
  ...
  const asideClass = mobileMode
    ? "ci-sidebar"
    : "ci-sidebar hidden lg:flex";

  return (
    <aside className={asideClass} style={wrapperStyle}>
      <style>{`
      ...
      <div className="ci-sidebar-scroll">
        <div className="ci-sidebar-label">Navigation</div>
      ...
      <div className="ci-sidebar-profile">
        ...
        <button className="ci-logout-btn" onClick={handleLogout}>
          <FiLogOut size={16} color="#64748B" />
        </button>
      </div>
    </aside>
  );
