...
import { FiChevronLeft, FiChevronRight, FiPlus, FiX, FiClock, FiBell, FiCheck, FiCalendar, FiAlertCircle, FiLoader } from "react-icons/fi";
...
function TodoCalendar() {
  ...
  const [form, setForm] = useState({ description: "", time: "12:00 am", notification: "" });
  ...
  useEffect(() => {
    ...
    if (msg) { setToast(msg); sessionStorage.removeItem(TOAST_KEY); setTimeout(() => setToast(""), 3000); }
  }, []);
  ...
  const getTodosForDate = (day) => {
    ...
    return todos.filter((t) => t.date === d || (t.type === "Repeat" && t.startDate <= d && t.completionDate >= d));
  };
  ...
  const handleSubmit = (e) => {
    ...
    else if (form.description.trim().length < 3) ne.description = "At least 3 characters";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-slate-900">To-Do Calendar</h1>
          <p className="text-sm text-slate-500 mt-1">Click any date to add a task</p>
        </div>
        <div className="flex items-center gap-2 bg-white border border-slate-200 rounded-2xl p-2 shadow-sm">
          <button onClick={goPrevMonth} className="cursor-pointer w-11 h-11 rounded-xl bg-slate-50 hover:bg-slate-100 flex items-center justify-center text-slate-700 transition">
            <FiChevronLeft size={20} />
          </button>
          <h2 className="min-w-[170px] text-center text-lg font-bold text-slate-900">{monthName}</h2>
          <button onClick={goNextMonth} className="cursor-pointer w-11 h-11 rounded-xl bg-slate-50 hover:bg-slate-100 flex items-center justify-center text-slate-700 transition">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="flex items-center justify-between px-6 pt-6 pb-4 flex-wrap gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">{todos.length} total tasks</p>
            </div>
          </div>
          <button onClick={goToday} className="cursor-pointer px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold transition-all duration-200 shadow-lg shadow-indigo-200">
            Today
          </button>
        </div>
        ...
        <div className="grid grid-cols-7 gap-px bg-slate-100 mx-6 rounded-xl overflow-hidden">
          {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => (
            <div key={day} className="bg-slate-50 text-center text-[11px] font-bold text-slate-400 uppercase py-2.5">{day}</div>
          ))}
        </div>
        ...
        <div className="p-6 pt-4">
          <div className="grid grid-cols-7 gap-2">
            {blankDays.map((_, i) => <div key={`b-${i}`} className="min-h-[100px]" />)}
                  ...
                  {dayTodos.length > 0 && (
                    <div className="space-y-1 mt-1.5">
                      {dayTodos.slice(0, 2).map((todo) => (
                        <div key={todo.id} className={`px-1.5 py-1 rounded-md text-[10px] font-semibold truncate leading-tight ${
                      ...
                      {dayTodos.length > 2 && (
                        <p className="text-[10px] font-semibold text-slate-400 pl-1">+{dayTodos.length - 2} more</p>
                      )}
                    </div>
                  )}
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl shadow-slate-900/20 w-full max-w-md overflow-hidden animate-[modalIn_0.3s_ease-out]">
            ...
            <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-slate-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-sm font-bold text-slate-800">Add for {selectedDate ? formatDisplay(selectedDate) : ""}</h3>
                  <p className="text-xs text-slate-400">Quick task entry</p>
                </div>
              </div>
              <button onClick={closeModal} className="w-8 h-8 rounded-lg border border-slate-200 bg-white flex items-center justify-center text-slate-400 hover:text-red-500 hover:border-red-200 hover:bg-red-50 transition-all">
                <FiX size={15} />
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                />
                {errors.description && touched.description && (
                  <p className="flex items-center gap-1 text-xs font-medium text-red-500 mt-1"><FiAlertCircle size={11} />{errors.description}</p>
                )}
              </div>

              {/* Time */}
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-500 tracking-wide uppercase flex items-center gap-1.5"><FiClock size={11} /> Time</label>
                <div className="relative">
                  ...
                  <FiChevronRight size={14} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none rotate-90" />
                </div>
              </div>

              {/* Notification */}
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-slate-500 tracking-wide uppercase flex items-center gap-1.5"><FiBell size={11} /> Notification</label>
                <div className="flex gap-2">
                  {["Yes", "No"].map((item) => (
                    <label key={item} className={`flex-1 flex items-center justify-center gap-2 p-2.5 rounded-xl border-2 cursor-pointer transition-all duration-200 ${
                      form.notification === item
                        ? item === "Yes" ? "border-emerald-400 bg-emerald-50 shadow-sm" : "border-slate-400 bg-slate-50 shadow-sm"
                        : "border-slate-200 bg-white hover:border-slate-300"
                    }`}>
                      <input type="radio" name="notification" value={item} checked={form.notification === item} onChange={handleChange} className="sr-only" />
                      <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${
                        form.notification === item ? (item === "Yes" ? "border-emerald-500" : "border-slate-500") : "border-slate-300"
                      }`}>
                        {form.notification === item && <div className={`w-2 h-2 rounded-full ${item === "Yes" ? "bg-emerald-500" : "bg-slate-500"}`} />}
                      </div>
                      <span className={`text-xs font-semibold ${form.notification === item ? (item === "Yes" ? "text-emerald-700" : "text-slate-700") : "text-slate-500"}`}>
                ...
                {errors.notification && touched.notification && (
                  <p className="flex items-center gap-1 text-xs font-medium text-red-500 mt-1"><FiAlertCircle size={11} />{errors.notification}</p>
                )}
              </div>
              ...
              <div className="flex gap-2.5 pt-2">
                ...
                <button type="submit" disabled={isSubmitting}
                  ...
                  {isSubmitting ? <><FiLoader size={14} className="animate-spin" /> Adding...</> : <><FiPlus size={14} /> Add Task</>}
                </button>
              </div>
            </form>
          </div>
